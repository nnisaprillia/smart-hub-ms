                ...
                <div class="p-6">
                                ...
                                <tbody class="bg-white divide-y divide-gray-200">
                                    ...
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                            <a href="{{ route('borrowings.show', $borrowing) }}" class="text-indigo-600 hover:text-indigo-900 mr-3">Lihat</a>
                                            @if($borrowing->status === 'pending')
                                            <form method="POST" action="{{ route('borrowings.approve', $borrowing) }}" class="inline mr-2">
                                                @csrf
                                                @method('PATCH')
                                                <button type="submit" class="text-green-600 hover:text-green-900 text-sm">Setujui</button>
                                            </form>
                                            @endif
                                            @if(in_array($borrowing->status, ['pending', 'approved']))
                                            <form method="POST" action="{{ route('borrowings.cancel', $borrowing) }}" class="inline" onsubmit="return confirm('Batalkan peminjaman ini?')">
                                                @csrf
                                                @method('PATCH')
                                                <button type="submit" class="text-red-600 hover:text-red-900 text-sm">Batal</button>
                                            </form>
                                            @endif
                                        </td>
                                    </tr>
                    ...
                    @else
                        <div class="text-center py-12">
                            ...
                            <p class="mt-1 text-sm text-gray-500">Belum ada permintaan peminjaman yang diajukan.</p>
                        </div>
                    @endif
                </div>
            </div>
